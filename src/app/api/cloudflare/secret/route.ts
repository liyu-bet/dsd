...
export async function GET(req: Request) {
  try {
    ...
    if (secretType !== 'password' && secretType !== 'apiToken' && secretType !== 'apiKey') {
    ...
    if (!encryptedValue) {
      return NextResponse.json({ error: 'Секрет не найден' }, { status: 404 });
    }

    const secret = await decryptAnySecret(encryptedValue);
    if (!secret) {
      return NextResponse.json({ error: 'Не удалось расшифровать секрет' }, { status: 500 });
    }

    return NextResponse.json({ secret });
  } catch (error) {
    console.error('Failed to read cloudflare secret:', error);
    return NextResponse.json({ error: 'Failed to read cloudflare secret' }, { status: 500 });
  }
}
