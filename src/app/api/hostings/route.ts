...
export async function POST(req: Request) {
  try {
    ...
    const item = await prisma.hostingAccount.create({
      data: {
        name,
        url,
        login,
    ...
    console.error("POST /api/hostings failed", error);
...
export async function PATCH(req: Request) {
  try {
    ...
    const existing = await prisma.hostingAccount.findUnique({ where: { id } });
    if (!existing) return NextResponse.json({ error: 'Hosting account not found' }, { status: 404 });
    ...
    console.error('PATCH /api/hostings failed', error);
