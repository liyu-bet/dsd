...
export async function GET(req: Request) {
  try {
    ...
    const account = await prisma.hostingAccount.findUnique({ where: { id: accountId }, select: { password: true, apiKey: true } });
    ...
    console.error('Failed to read hosting secret:', error);
    return NextResponse.json({ error: 'Failed to read hosting secret' }, { status: 500 });
  }
}
